...
<template>
    <GuestLayout>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2">
                <label class="flex items-center gap-2 text-sm text-muted-foreground">
                    <input type="checkbox" v-model="form.remember" class="size-4 rounded border-input accent-[var(--primary)]" />
                    تذكّرني
                </label>
                <Link v-if="canResetPassword" :href="route('password.request')"
                    class="text-sm font-medium text-primary hover:underline">نسيت كلمة المرور؟</Link>
            </div>

            <Button type="submit" class="w-full gap-2" variant="accent" :loading="form.processing">
        ...
        <a href="/auth/google/redirect"
            class="flex h-10 w-full items-center justify-center gap-2 rounded-md border border-input bg-card text-base font-semibold transition-colors hover:border-primary/40 hover:bg-muted">
